...
describe('AuthController', () => {
  ...
  describe('root', () => {
    it('should login', async (done) => { 
      ...
      jest.spyOn(authService, 'validateUser').mockImplementation(()=> Promise.resolve({...testUser, password: criptedPass}));
    ...
    it('should is Logged In', async (done) => { 
     const isLoggedIn = await authController.isLoggedIn({jwt: authdata.accessToken});
      expect(isLoggedIn).toBeTruthy();
      done();
    });

    it('should is not Logged In', async (done) => { 
      const isNotLoggedIn = await authController.isLoggedIn({jwt: 'wrong-token'});
      expect(isNotLoggedIn).toBeFalsy();
       done();
     });

    it('should return userdata', async (done) => {
      const userdata = await authController.getUserData({jwt: authdata.accessToken});
