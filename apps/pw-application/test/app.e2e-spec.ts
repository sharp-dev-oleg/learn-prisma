...
describe('app (e2e)', () => {
  ...
  it('/registration (POST)', () => {
    ...
    jest.spyOn(authService,'registration').mockImplementation(() => Promise.resolve({id:1}));
  ...
  it('/login (POST)', () => {
    ...
    jest.spyOn(authService,'login').mockImplementation(() => Promise.resolve({userId:1, accessToken:'token'}));
    return request(app.getHttpServer())
      .post('/login')
      .set('Accept','application/json')
      .send(loginDate)
  ...
  it('/userdata (GET)',  () => {
    ...
    jest.spyOn(authService,'getUserData').mockImplementation(() => Promise.resolve({id:1, username:'test', exp:123456}));
    return request(app.getHttpServer())
      .get('/userdata')
      .set('Accept','application/json')
  ...
  it('/wailets/:userId (GET)',  () => {
    const token = 'token';
    jest.spyOn(authClient,'send').mockImplementation(()=>of(true)) 
    jest.spyOn(pwService,'getWailets').mockImplementation(() => Promise.resolve([]));
    return request(app.getHttpServer())
      .get('/wailets/1')
      .set('Accept','application/json')
      .set('Authorization',`Bearer ${token}`)
  ...
  it('/recent (GET)',  () => {
    const token = 'token';
    jest.spyOn(authClient,'send').mockImplementation(()=>of(true)) 
    jest.spyOn(pwService,'getTransaction').mockImplementation(() => Promise.resolve([]));
    return request(app.getHttpServer())
      .get('/recent')
      .set('Accept','application/json')
      .set('Authorization',`Bearer ${token}`)
  ...
  it('/send (POST)',  () => {
    ...
    jest.spyOn(authClient,'send').mockImplementation(()=>of(true)) 
    jest.spyOn(pwService,'sendTransaction').mockImplementation(() => Promise.resolve());
    return request(app.getHttpServer())
      .post('/send')
      .set('Accept','application/json')
      .set('Authorization',`Bearer ${token}`)
  ...
  it('/search?q=fr (GET)', () => {
    const token = 'token';
    jest.spyOn(authClient,'send').mockImplementation(()=>of(true)) 
    jest.spyOn(userService,'search').mockImplementation(() => Promise.resolve([]));
    return request(app.getHttpServer())
      .get('/search?q=fr')
      .set('Accept','application/json')
      .set('Authorization',`Bearer ${token}`)
