...
@WebSocketGateway()
export class GetWayController implements OnGatewayConnection, OnGatewayDisconnect{
  ...
  constructor(private readonly service: PwService, private readonly authService: AuthService) {}

  @UseGuards(AuthWSGuard)
  @SubscribeMessage('get_my_wailets')
  getMyWailets(client: Socket): Observable<WsResponse<Array<Wailet>>> {
    return this.getUser(client.request).pipe(take(1),switchMap((user)=>from(this.service.getWailets(user.id))),
  ...
  handleConnection(client: Socket, ...args: any[]) {
    ...
    this.getUser(client.handshake).pipe(take(1),filter(user=>user !== false)).subscribe(user=>{
 ...
 async updateTransaction(@MessageBody() model:Transaction) {
    ...
    Logger.log(model);
     ...
     if(this.users[from.userId] !== undefined ){
      Logger.log(`send transaction update to ${from.userId} - ${this.users[from.userId].id}`); 
     ...
     if(this.users[to.userId] !== undefined ){
      ...
      Logger.log(`send transaction update to ${to.userId} - ${this.users[to.userId].id}`);
  ...
  getUser(req: any) {
    ...
    return  from(this.authService.getUserData(req.headers['authorization']?.split(' ')[1]));
  }
